...
(defn create_index_writer [conf]
  "Creates an (in memory) index and exposes/returns interesting structures as a hash."

      (def _dir (or (conf :dir) (RAMDirectory.)))

      (def analyzer (if (:analyzer conf) (:analyzer conf) (StandardAnalyzer. version)))

      (def config (IndexWriterConfig. version analyzer))

      (def writer (IndexWriter. _dir config))
...
(defn create_index_reader [conf]
  ...
  (def _dir (or (conf :dir) (throw (IllegalArgumentException. (str "the :dir parameter is required")))))

  (def analyzer (or (:analyzer conf) (StandardAnalyzer. version)))

  (def config (IndexWriterConfig. version analyzer))

  (def reader (IndexReader/open _dir true))
